...
async def diarize_endpoint(video_id: str):
    ...
    """
    print("DEBUGGING: Error opening input file 1")
    ...
    print(f"\t{title}")
    # f"ffmpeg -i {video_path} -vn -acodec pcm_s16le -ar 16000 -y {audio_path}".split(' ')
    subprocess.run(
        [
            "ffmpeg",
            "-i", f"{video_path}",
    ...
    #
    diar_segments = _alignment_service.diarize(str(audio_path))
    print("DEBUGGING: DIAR SEGMENTS")
    print(f"\t{diar_segments}")
